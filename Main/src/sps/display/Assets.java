...
public class Assets {
    ...
    private Assets() {
        _fonts = new FontPack();
        for (FileHandle spriteTile : Loader.get().assetDir("graphics/sprites").list()) {
            if (!spriteTile.file().isHidden()) {
                String[] comps = spriteTile.name().split("-");
                ...
                Sprite sprite = new Sprite(new Texture(spriteTile.path()));
    ...
    private FileHandle fragmentShader() {
        return Loader.get().graphics("fragment.glsl");
    }

    private FileHandle vertexShader() {
        return Loader.get().graphics("vertex.glsl");
    }
    ...
    public Texture image(String fileName) {
        if (!textures.containsKey(fileName)) {
            textures.put(fileName, new Texture(Loader.get().graphics(fileName).path()));
        }
